...
if($action == "like_comment"){
	$save = $crud->like_comment();
	if($save)
		echo $save;
}

if($_GET['action'] == 'save_class'){
...
if ($_GET['action'] == 'get_class_categories') {
  $id = intval($_GET['id']);
  $res = $conn->query("SELECT category_ids FROM live_classes WHERE id = $id");
  $row = $res->fetch_assoc();
  echo json_encode(explode(",", $row['category_ids']));
  exit;
}
